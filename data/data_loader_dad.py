...
class NASA_Anomaly(Dataset):
    ...
    def __read_data__(self):
        ...
        if self.flag == 'train':
            data = np.load(os.path.join(self.root_path, self.data_path, '{}_train.npy'.format(self.data_path)))
            data = data.reshape((-1, x_dim))
        elif self.flag in ['val', 'test']:
            try:
                data = np.load(os.path.join(self.root_path, self.data_path, '{}_test.npy'.format(self.data_path)))
                data = data.reshape((-1, x_dim))
            except (KeyError, FileNotFoundError):
                data = None
            try:
                label = np.load(os.path.join(self.root_path, self.data_path, '{}_test_label.npy'.format(self.data_path)))
